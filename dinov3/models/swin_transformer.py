...
class WindowAttention(nn.Module):
    ...
    def compute_attention(self, qkv, attn_bias=None):
        B_, N, C = qkv.shape
        q, k, v = qkv.reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
    ...
    def forward_list(self, x_list, attn_bias=None):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
        ...
        attn_out = []
        for qkv in qkv_list:
            x_, attn_out_ = self.compute_attention(qkv, attn_bias)
            x.append(x_)
            attn_out.append(attn_out_)
        x_flat, shapes, num_tokens = cat_keep_shapes(x)
        x_flat = self.proj(x_flat)
        x = self.proj_drop(x_flat)
        x = uncat_with_shapes(x_flat, shapes, num_tokens)
        return x, attn_out
    ...
    def _forward(self, x):
        ...
        if self.shift_size > 0:
            ...
            if H is self.attn_mask_dict.keys():
                ...
                self.attn_mask_dict[H] = self.create_attn_mask(H, W).to(x.device).to(x.dtype)
                attn_mask = self.attn_mask_dict[H]
        ...
        # W-MSA/SW-MSA
        attn_windows, attn = self.attn(x_windows, attn_mask)  # B*nW, window_size*window_size, C
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 return_all_tokens=False, use_mean_pooling=True, masked_im_modeling=True, 
                 n_storage_tokens: int = 0, untie_cls_and_patch_norms: bool = True,
        ...
        self.proj_cls = nn.Linear(self.num_features, self.num_features)
        self.untie_cls_and_patch_norms = untie_cls_and_patch_norms
        if untie_cls_and_patch_norms:
            # When untying, this norm is applied to CLS tokens and registers.
            self.cls_norm = norm_layer(self.num_features)
        else:
            self.cls_norm = None
        ...
        if untie_global_and_local_cls_norm:
            ...
            self.local_cls_norm = norm_layer(self.num_features)
        else:
        ...
        if masked_im_modeling:
            self.masked_embed = nn.Parameter(torch.zeros(1, embed_dim, device=device))
    ...
    def forward_(self, x, mask=None):
        ...
        x_norm_patch = self.norm(x)  # B L C
        x = self.avgpool(x_norm_patch.transpose(1, 2))  # B C 1
        x = torch.flatten(x, 1)
        x_cls = self.proj_cls(x).unsqueeze(1)
        if self.untie_cls_and_patch_norms:
            x_norm_cls_reg = self.cls_norm(x_cls)
        else:
            x_norm_cls_reg = self.norm(x_cls)
        ret = {
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor]) -> List[Dict[str, Tensor]]:
        x = []
        for x_, mask_ in zip(x_list, masks_list):
            # patch linear embedding
            x_ = self.patch_embed(x_)
            # mask image modeling
            if mask_ is not None:
                x_ = self.mask_model(x_, mask_)
            x_ = x_.flatten(2).transpose(1, 2)

            if self.ape:
                x_ = x_ + self.absolute_pos_embed
            x_ = self.pos_drop(x_)
            x.append(x_)
        for layer in self.layers:
            x = layer(x)
        output = []
        for idx, (x_, masks) in enumerate(zip(x, masks_list)):
            x_norm_patch = self.norm(x_)  # B L C
            x_cls = self.avgpool(x_norm_patch.transpose(1, 2))  # B C 1
            x_cls = torch.flatten(x_cls, 1)
            x_cls = self.proj_cls(x_cls).unsqueeze(1)
            if self.untie_cls_and_patch_norms or self.untie_global_and_local_cls_norm:
                if self.untie_global_and_local_cls_norm and self.training and idx == 1:
                    x_norm_cls_reg = self.local_cls_norm(x_cls)
    ...
    def forward(self, x, masks=None, is_training: bool = True):
        if type(x) == list:
            return self.forward_features_list(x, masks)
        else:
            return self.forward_(x, mask=masks)


    def forward_list_demo(self, x_list):
        masks_list = [None] * len(x_list)
        return self.forward_features_list(x_list, masks_list)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i >= start_stage:
                for x_ in fea[start_blk:]:
                    ...
                    output.append({
                        "x_norm_clstoken": x_cls if x_cls is not None else x_avg,
    ...
    def init_weights(self, pretrained='', pretrained_layers=[], verbose=True):
        ...
        if pretrained == '':
            if self.masked_im_modeling:
                # nn.init.zeros_(self.masked_embed)
                trunc_normal_(self.masked_embed, std=0.0005, a=-0.0015, b=0.0015)
            self.apply(self._init_weights)
    ...
    def mask_model(self, x, mask):
        if len(mask.shape) == 2:
            spatial_size = int(mask.shape[1] ** 0.5)
            mask = mask.reshape(-1, spatial_size, spatial_size)
    ...
    def prepare_tokens_with_masks(self, x, mask):
        # extend mask for hierarchical features
        if x.shape[-2:] != mask.shape[-2:]:
            htimes, wtimes = np.array(x.shape[-2:]) // np.array(mask.shape[-2:])
            mask = mask.repeat_interleave(htimes, -2).repeat_interleave(wtimes, -1)
...
if __name__ == "__main__":
    # 测试 SwinTransformer 的 forward_list 函数
    # model = SwinTransformer(
    #     img_size=448,
    #     patch_size=4,
    #     in_chans=3,
    ...
    #     num_heads=[3, 6, 12, 24],
    #     window_size=7,
    #     ape=False,  # 关闭绝对位置编码，以支持不同尺寸的输入
    # )
    model = swin_tiny().cuda()
    model.eval()

    # 构造长度为 2 的 List[Tensor]
    x_list = [
        torch.randn(16, 3, 448, 448).cuda(),  # 第一个 Tensor
        torch.randn(64, 3, 224, 224).cuda(),    # 第二个 Tensor
    ]
    ...
    with torch.no_grad():
        output = model.forward_list(x_list)
