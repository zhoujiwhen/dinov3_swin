...
def activation_checkpoint_transformer_swin(cfg, model: nn.Module):
    _checkpointing_wrapper = get_activation_checkpoint_wrapper(cfg)
    for block_id, b in enumerate(model.layers):
        model.layers[block_id] = _checkpointing_wrapper(b)
...
def ac_compile_parallelize(
    ...
    ARCH_TYPE_MAP = {
        ...
        SwinTransformer: dict(
            compile_fn=compile_transformer_swin,
            fsdp_fn=fsdp_transformer_swin,
            activation_checkpointing_fn=activation_checkpoint_transformer_swin,
        ),
    }
